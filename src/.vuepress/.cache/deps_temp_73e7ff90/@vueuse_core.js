...
function useArrayIncludes(...args) {
  ...
  return computed2(() => toValue(list).slice(formIndex).some((element, index, array) => comparator(
    toValue(element),
    toValue(value),
    index,
    toValue(array)
...
function useArrayJoin(list, separator) {
  return computed2(() => toValue(list).map((i) => toValue(i)).join(toValue(separator)));
}
...
function watchOnce(source, cb, options) {
  const stop2 = watch2(source, (...args) => {
    nextTick(() => stop2());
    return cb(...args);
  }, options);
  return stop2;
...
function onElementRemoval(target, callback, options = {}) {
  ...
  const stopWatch = watchEffect(() => {
    const el = unrefElement(target);
    if (el) {
      const { stop: stop2 } = useMutationObserver(
        document2,
        (mutationsList) => {
...
function useDisplayMedia(options = {}) {
  ...
  async function _start() {
    ...
    (_a2 = stream.value) == null ? void 0 : _a2.getTracks().forEach((t) => useEventListener(t, "ended", stop2, { passive: true }));
    return stream.value;
  }
